...
class LogController extends Controller
{
    ...
    public function getLog(Request $request)
    {
        ...
        {  
            ...
            {
                $log_group = User::SeachRecordsbyWhere("log_group", "id_log=? and id_group=?", [$id_log_disc_flow, $request->input('id_group')],"id,id_teacher_edit,log_group_json")[0];
                return response()->json(["id_log_group"=>$log_group->id,"id_teacher_edit"=>$log_group->id_teacher_edit, "log"=>json_decode($log_group->log_group_json)]);
            }    
        } 
    ...
    public function ResultUpdateLog(Request $request)
    {
        ...
        $token_verification = GeneralUserController::VerifactionToken($request->input('id_user_reg'),$request->input('token'));
        if (count($token_verification)==0) return response()->json(["error" => "Вы не авторизованы. Войдите в систему."]);
        else {  User::UpdateColumn("log_group",['id','=',$request->input('id_log_group')], ["log_group_json"=>$request->input('log'),"id_teacher_edit"=>null]); } 
    }
